...
func wsHandler(w http.ResponseWriter, r *http.Request) {
	...
	for {
		...
		switch msg.Type {
		case server.MsgCreatePlayer:
			server.GlobalHandler.CreatePlayer(msg.Content, conn)
		case server.MsgCreatRoom:
			server.GlobalHandler.CreateRoom(msg.Content)
		case server.MsgReady:
			data, _ := server.UnmarshalContent(msg.Content)
			server.GlobalHandler.Ready(data["playerName"].(string), data["roomID"].(string))
		case server.MsgUnReady:
			data, _ := server.UnmarshalContent(msg.Content)
			server.GlobalHandler.UnReady(data["playerName"].(string), data["roomID"].(string))
		case server.MsgJoinRoom:
			fmt.Println("接收到的加入房间消息", msg.Content)
			data, _ := server.UnmarshalContent(msg.Content)
			server.GlobalHandler.JoinRoom(data["playerName"].(string), data["roomID"].(string))
		case server.MsgLeaveRoom:
			data, _ := server.UnmarshalContent(msg.Content)
			server.GlobalHandler.LeaveRoom(data["playerName"].(string), msg.RoomID)
		case server.MsgSetFirst:
			data, _ := server.UnmarshalContent(msg.Content)
